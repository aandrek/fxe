...
export default class Fxengine {
    ...
    fxBPMCallFn(fn, offsetPercentage, beatMeasure)
    {
        ...
        if(typeof this.cacheWindowIdentifiers[offsetPercentage + '-' + beatMeasure] == 'undefined' || this.cacheWindowIdentifiers[offsetPercentage + '-' + beatMeasure] != id){
    ...
    fxBPMInterpolation(minValue, maxValue, fxOffsetSeed, beatMeasure, easingFn, loop)
    {
        ...
        let res = minValue + (posibleInc * easingFn(this.getCurrentFxWindowPercent(fxOffsetSeed, beatMeasure, loop)));
        return res;
    };

    // ---------------------------------------------------------------------------

    fxBPMWaveInterpolation(minValue, maxValue, fxOffsetSeed, beatMeasure, easingFn)
    {
        if(typeof fxOffsetSeed == 'undefined' || _.isNull(fxOffsetSeed)){
            fxOffsetSeed = 0;
        }
